...
Eine Anmeldung davor braucht es nicht: liegt keine vor, zeigt das Skript
einen Gerätecode und läuft danach weiter. Wer schon angemeldet ist, wird
nicht erneut gefragt.

### Warum der Gerätecode

Über den Windows-Kontenmanager (WAM) endet die Anmeldung auf PowerShell 7.6
so:
...
meldet sich im Browser an, und das Skript laeuft weiter wie gehabt. Das
Skript waehlt ihn deshalb von sich aus, wenn es sich selbst anmeldet.
